(function(factory) {
	...
	$.extend($.jgrid, {
		...
		parseDate: function(format, date, newformat, opts) {
			var token = /\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g,
				...
				offset = 0;
			if(opts === undefined) {
				opts = $.jgrid.getRegional(this, "formatter.date")
			}
		...
		clearBeforeUnload: function(jqGridId) {
			...
			if($.isFunction(grid.emptyRows)) {
				grid.emptyRows.call($t, true, true)
			}
		...
		return this.each(function() {
			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
				...
				setPager = function(pgid, tp) {
					...
					cent = pgid + "_center";
					rgt = pgid + "_right";
					$("#" + $.jgrid.jqID(pgid)).append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, "pagerTable", false, "ui-pg-table ui-common-table ui-pager-table") + "><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>").attr("dir", "ltr");
			...
			var arf = ts.p.hidegrid === true ? $("<a role='link' class='ui-jqgrid-titlebar-close HeaderButton " + cornerall + "' title='" + ($.jgrid.getRegional(ts, "defaults.showhide", ts.p.showhide) || "") + "'" + " />").hover(function() {
				arf.addClass(hover)
			}, function() {
			...
			return this.each(function() {
				...
				if(loadDiv.is(":visible")) {
					...
					if(loadDiv.offset().top < scrollTop) {
						top = Math.min(10 + scrollTop - offsetParent.offset().top, offsetParent.height() - loadDiv.height());
	...
	$.extend($.jgrid, {
		...
		info_dialog: function(caption, content, c_b, modalopt) {
			...
			cnt += c_b ? "<div class='" + classes.header + "' style='text-align:" + mopt.buttonalign + ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a id='closedialog' class='fm-button " + common.button + "'>" + c_b + "</a>" + buttstr + "</div>" : buttstr !== "" ? "<div class='" + classes.header + "' style='text-align:" + mopt.buttonalign + ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>" + buttstr + "</div>" : "";
			cnt += "</div>";
			try {
	...
	$.fn.jqFilter = function(arg) {
		...
		return this.each(function() {
			...
			this.createTableRowForRule = function(rule, group) {
				...
				$(ruleDataInput).addClass("input-elm " + classes.srInput).bind("change", function() {
			...
			this.getStringForRule = function(rule) {
				var opUF = "",
					opC = "",
					i, cm, ret, val, numtypes = ["int", "integer", "float", "number", "currency"];
		...
		searchGrid: function(p) {
			...
			return this.each(function() {
				...
				if($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
					...
					showFilter($("#" + fid));
					$(".fm-button:not(." + common.disabled + ")", fil).hover(function() {
						$(this).addClass(common.hover)
					}, function() {
						$(this).removeClass(common.hover)
					})
	...
	$.jgrid.extend({
			...
			return this.each(function() {
				...
				function updateNav(cr, posarr) {
					...
					if(cr === 0) {
						...
						if(posarr[1][cr - 1] !== undefined && $("#" + $.jgrid.jqID(posarr[1][cr - 1])).hasClass(commonstyle.disabled)) {
							$("#pData", frmtb2).addClass(commonstyle.disabled)
						} else {
		...
		viewGridRow: function(rowid, p) {
			...
			return this.each(function() {
				...
				$(".fm-button:not(." + commonstyle.disabled + ")", "#" + frmtb + "_2").hover(function() {
					$(this).addClass(commonstyle.hover)
				}, function() {
					$(this).removeClass(commonstyle.hover)
				});
			...
			return this.each(function() {
				...
				if($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
					...
					$("#dData", "#" + dtbl + "_2").click(function() {
						var ret = [true, ""],
							pk, postdata = $("#DelData>td", "#" + dtbl).text();
						onCS = {};
	...
	$.jgrid.extend({
		...
		setGroupHeaders: function(o) {
			...
			return this.each(function() {
				var ts = this,
					i, cmi, skip = 0,
					$tr, $colHeader, th, $th, thStyle, iCol, cghi, numberOfColumns, titleText, cVisibleColumns, className, colModel = ts.p.colModel,
					cml = colModel.length,
					ths = ts.grid.headers,
				...
				for(i = 0; i < cml; i++) {
					...
					if(iCol >= 0) {
						...
						$colHeader = $("<th>").attr({
							role: "columnheader"
						}).addClass(base.headerBox + " ui-th-column-header ui-th-" + ts.p.direction + " " + className).html(titleText);
	...
	$.jgrid.extend({
		pivotSetup: function(data, options) {
			...
			this.each(function() {
				var row, rowindex, i, rowlen = data.length,
									...
									if(lastval[items.level] !== items.text && items.children.length && items.text.indexOf("_r_Totals") === -1) {
										if(items.level > 0) {
											...
											var collen = headers[items.level - 1].groupHeaders.length - 1,
												colpos = collen === 0 ? swaplen : initColLen;
			...
			return this.each(function() {
				...
				while(i < len) {
					...
					if(curLevel !== parseInt($t.p.tree_root_level, 10)) {
						expan = $($t).jqGrid("isVisibleNode", ldat);
						if(!expan) {
