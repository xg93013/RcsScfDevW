...
public class AvfFinancingRelConfigureHandler {
    ...
    public List<AvfFinancingRelConfigure> findCompanyByFinCode(String token) {
        Map<String,String> param = new HashMap<>();
        param.put("token",token);
        String result = HttpClientUtil.httpKvPost(ServerConstant.URL_AVF_FINANCINGRELCONFIGURE_COMPANY_FINCODE, param);
        JSONObject jsonObject = JSONObject.parseObject(result);
        HttpClientUtil.isOk(jsonObject);
        return JSONObject.parseObject(jsonObject.getString("data"),new TypeReference<List<AvfFinancingRelConfigure>>(){});
    }

    public List<AvfFinancingRelConfigure> findSupplyByFinCodeAndCompanyCode(String token, String companyCode) {
        Map<String,String> param = new HashMap<>();
        param.put("token",token);
        param.put("companyCode",companyCode);
        String result = HttpClientUtil.httpKvPost(ServerConstant.URL_AVF_FINANCINGRELCONFIGURE_SUPPLY_COMPANYFINCODE, param);
        JSONObject jsonObject = JSONObject.parseObject(result);
        HttpClientUtil.isOk(jsonObject);
        return JSONObject.parseObject(jsonObject.getString("data"),new TypeReference<List<AvfFinancingRelConfigure>>(){});
    }
