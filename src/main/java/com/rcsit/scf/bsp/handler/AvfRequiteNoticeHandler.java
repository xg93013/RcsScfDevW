...
public class AvfRequiteNoticeHandler {
    ...
    public JSONArray repaymentQuota(String token, String datetime) {
        String result = HttpClientUtil.httpGets(ServerConstant.URL_AVF_REQUITE_QUOTA, "token=" + token + "&datetime=" + datetime);
        JSONObject jsonObject = JSONObject.parseObject(result);
        HttpClientUtil.isOk(jsonObject);
