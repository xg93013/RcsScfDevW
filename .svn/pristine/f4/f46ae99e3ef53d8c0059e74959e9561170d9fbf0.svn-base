...
public class PbmCompanyController extends BaseController {
    ...
    public String companyDetail(Model model, @RequestParam("companyCode")String companyCode, @CookieValue("token")String token){
        String data = pbmCompanyHandler.doCompanyDetail(companyCode,token);
        PbmCompany pc = JSONObject.parseObject(data, new TypeReference<PbmCompany>(){});
        model.addAttribute("company",pc);
        return "avf/enterprise/company_detail";
    }
}
