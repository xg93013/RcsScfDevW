...
public class ReadExcelUtil {
    ...
    public static Map<Integer, List<String>> upload(MultipartFile file) {
        ...
        for (int r = 0; r < rowCount; r++) {
            ...
            if (null == row || null==row.getCell(0) || StringUtils.isEmpty(row.getCell(0).getStringCellValue())) {
                continue;
            }
            ...
            for (int c = 0; c < row.getPhysicalNumberOfCells(); c++) {
                ...
                if(null == cell || StringUtils.isEmpty(cellValue)){
                    cellValue="";
                }else {
        ...
        switch (cell.getCellType()) {
            case HSSFCell.CELL_TYPE_NUMERIC:                        //数值型
                if(HSSFDateUtil.isCellDateFormatted(cell)){ //如果是日期格式
                    ...
                    value = sdf.format(date);  //把读取日期转换为文本
