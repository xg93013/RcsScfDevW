...
public class HttpClientUtil {
    ...
    public static String httpKvPost(String url, Object data) {
        ...
        try {
            ...
            HttpEntity entity = response.getEntity();
            String value = EntityUtils.toString(entity, "utf-8");
//            Optional.ofNullable(value).orElseThrow(RequestErrorException::new);
            if (StringUtils.isEmpty(value))
                value = requestEmptyResult();
            return value;
        } catch (Exception e) {
    ...
    public static boolean isOk(JSONObject jsonObject) {
        if (jsonObject.getInteger("status") == (HttpStatus.OK.value()))
            return true;
        else if (jsonObject.getInteger("status") == HttpStatus.NETWORK_AUTHENTICATION_REQUIRED.value())
            throw new TokenErrorException();
        throw new RequestErrorException(jsonObject.getString("message"));
    }


    private static String requestEmptyResult() {
